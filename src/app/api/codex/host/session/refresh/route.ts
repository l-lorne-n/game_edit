...
export async function POST(request: Request) {
  try {
    ...
    await service.init();
    const response = await service.refresh(body.sessionId, body.previousAccountId ?? undefined);
    return NextResponse.json(response);
  } catch (error) {
    const status = error instanceof HostTokenRouteAuthError ? error.status : 500;
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to refresh host token session' },
      { status },
    );
  }
}
